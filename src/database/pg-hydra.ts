...
export class PGHydraDatabase {
  private pool: Pool;
  private dbConfig: any;
  private sharded: boolean;

  constructor(configOverride?: any, sharded: boolean = false) {
  ...
  async connect(): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Check if PG Hydra extension is available
        if (this.sharded) {
          await client.query('SELECT hydra_shard_version()');
          console.log('✅ PG Hydra sharding extension detected');
        }
        ...
        console.log(`PG Hydra connection attempt ${attempt}/${maxRetries} failed: ${lastError.message}`);
    ...
    throw new Error(`Failed to connect to PG Hydra after ${maxRetries} attempts: ${lastError?.message}`);
  }
  ...
  async ensureDatabaseExists(): Promise<void> {
    ...
    try {
      ...
      if (result.rows.length === 0) {
        ...
        console.log(`Created PG Hydra database: ${this.dbConfig.database}`);
      }
    } catch (error) {
      console.log(`PG Hydra database creation failed: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
  ...
  async createTable(): Promise<void> {
    ...
    // Create sharding configuration if sharding is enabled
    if (this.sharded) {
  ...
  async dropTable(): Promise<void> {
    // Drop sharding configuration first if sharding is enabled
    if (this.sharded) {
      await this.cleanupSharding();
    }
  ...
  private async setupSharding(): Promise<void> {
    try {
      // Create sharding configuration for the performance_test table
      // This is PG Hydra-specific setup
      await this.query(`
        SELECT hydra_create_shard_table(
          'performance_test',
          'timestamp',
          'hash',
          4  -- Number of shards
      ...
      console.log('✅ PG Hydra sharding configured for performance_test table');
    } catch (error) {
      console.log(`⚠️  PG Hydra sharding setup failed (may not be available): ${error instanceof Error ? error.message : String(error)}`);
    }
  }

  private async cleanupSharding(): Promise<void> {
    try {
      // Clean up sharding configuration
      await this.query(`SELECT hydra_drop_shard_table('performance_test')`);
      console.log('✅ PG Hydra sharding configuration cleaned up');
    } catch (error) {
      console.log(`⚠️  PG Hydra sharding cleanup failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async getShardInfo(): Promise<any> {
    if (!this.sharded) {
      return { sharded: false, message: 'Sharding not enabled' };
    }
